...
head(heart.dt2)
targs <- heart.dt2[, num]
# this selects all columns except num
...
te.feats <- heart.dt2[-trainIdx, -'num']
te.targs <- heart.dt2[-trainIdx]$num
...
sse <- sum((te.preds$pred - tr.targs) ^ 2)
...
# library(kknn)
# res <- kknn(formula = num ~ ., rtrain = tr.feats, test = te.feats)
